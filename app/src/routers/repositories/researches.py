...
class ResearchRepository:
    ...
    def get_researches(
        ...
        query = db.query(Research).join(Research.author)
        if status:
        ...
        if search_text:
            query = query.filter(
                or_(
                    Research.title.ilike(f"%{search_text}%"),
                    Research.description.ilike(f"%{search_text}%"),
                )
            )

        if category_ids:
            category_list = [int(id) for id in category_ids.split(",")]
            query = query.join(ResearchCategories).filter(
                ResearchCategories.category_id.in_(category_list)
            )

        researches = query.order_by(Research.created_at.desc()).limit(limit).offset(offset).all()
        return researches
    ...
    def create_research(db: Session, research_data: ResearchCreateRequest, user_id: int):
        ...
        for category_id in category_id_list:
            db_category = ResearchCategories(research_id=db_research.id, category_id=category_id)
            db.add(db_category)
    ...
    def get_by_id(db: Session, research_id: int):
        return (
            db.query(Research)
            .filter(Research.id == research_id, Research.status == Status.PUBLISHED)
            .first()
        )
    ...
    def update(db: Session, db_research: Research, research: ResearchUpdateRequest):
        ...
        if research.category_ids:
            ...
            for category_id in categories:
                ...
                db.query(ResearchCategories).filter(
                    ResearchCategories.research_id == db_research.id
                ).delete()

                # then, add the categories to the research
                db_category = ResearchCategories(
                    research_id=db_research.id, category_id=category_id
                )
                db.add(db_category)
            db.commit()
            db.refresh(db_research)
